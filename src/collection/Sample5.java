...
public class Sample5 {
	public static void main(String[] args) {
		...
		//	순서는 존재하지 않고(그래서 hash가능) value는 중복이 가능하지만 key는 중복될 수 없다.
		Map<String, Student> m=new HashMap<String, Student>();
		//Map<key,value> generic 지정 시 두개를 지정해야 한다.
		//Map.put(key,value); key 와 value를 한쌍으로 묶어 데이터 입력
		//Map.get(key);		검색어를 기준으로 해당하는 value를 출력
		m.put("B", new Student("김철수",105));
		m.put("A", new Student("최지향",110));
...
//		m.put("C", 999);	//key는 중복되면 이전의 값이 소멸한다
		...
		//Map을 for문으로 운행하기 위해서는 key들이 들어가있는 Set을 추출한 뒤에 사용하여아 한다.
	}
}
class Student{
	String name;
	...
	//따라서 객체 내부의 toString()을 오버라이드하면 원하는 내용이 콘솔에 출력되게 수정 가능하다.
